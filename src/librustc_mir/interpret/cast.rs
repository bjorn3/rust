    ...
    fn unsize_into_ptr(
        ...
        match (&src_pointee_ty.kind, &dest_pointee_ty.kind) {
            ...
            (_, &ty::Dynamic(ref data, _)) => {
                ...
                if src_pointee_ty.needs_subst() || data.principal().needs_subst() {
                    throw_inval!(TooGeneric);
                }

                let vtable =
                    self.tcx.get_vtable(src_pointee_ty, data.principal())?;
                let vtable = self.memory.tag_static_base_pointer(vtable);
                let ptr = self.read_immediate(src)?.to_scalar_ptr()?;
