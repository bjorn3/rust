...
impl CliCrateType {
    ...
    pub fn all() -> &'static [(Symbol, Self)] {
        debug_assert_eq!(CliCrateType::default(), CliCrateType::Rlib);
        &[
            (rustc_span::sym::lib, CliCrateType::Rlib),
            (rustc_span::sym::rlib, CliCrateType::Rlib),
            (rustc_span::sym::dylib, CliCrateType::Dylib),
            (rustc_span::sym::cdylib, CliCrateType::Cdylib),
            (rustc_span::sym::staticlib, CliCrateType::StaticLib),
            (rustc_span::sym::proc_dash_macro, CliCrateType::ProcMacro),
            (rustc_span::sym::bin, CliCrateType::Executable),
            (rustc_span::sym::sdylib, CliCrateType::Sdylib),
        ]
    }
...
impl TryFrom<Symbol> for CliCrateType {
    type Error = ();
    ...
    fn try_from(value: Symbol) -> Result<Self, Self::Error> {
        Ok(match value {
            ...
            rustc_span::sym::cdylib => CliCrateType::Cdylib,
            rustc_span::sym::rlib => CliCrateType::Rlib,
            rustc_span::sym::lib => CliCrateType::default(),
            rustc_span::sym::proc_dash_macro => CliCrateType::ProcMacro,
            rustc_span::sym::sdylib => CliCrateType::Sdylib,
            _ => return Err(()),
        })
...
impl std::fmt::Display for CliCrateType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            CliCrateType::Executable => "bin".fmt(f),
            CliCrateType::Dylib => "dylib".fmt(f),
            CliCrateType::Rlib => "rlib".fmt(f),
            CliCrateType::StaticLib => "staticlib".fmt(f),
            CliCrateType::Cdylib => "cdylib".fmt(f),
            CliCrateType::ProcMacro => "proc-macro".fmt(f),
            CliCrateType::Sdylib => "sdylib".fmt(f),
        }
    }
}

impl IntoDiagArg for CliCrateType {
...
impl CrateTypes {
    pub fn only_rlib(&self) -> bool {
        match self {
            CrateTypes::Executable | CrateTypes::ProcMacro | CrateTypes::Sdylib => false,
            CrateTypes::Lib(tys) => tys.iter().all(|ty| *ty == LibCrateType::Rlib),
        }
    }

    pub fn any_rlib(&self) -> bool {
        match self {
            CrateTypes::Executable | CrateTypes::ProcMacro | CrateTypes::Sdylib => false,
            CrateTypes::Lib(tys) => tys.iter().any(|ty| *ty == LibCrateType::Rlib),
        }
    }
    ...
    pub fn never_linked(&self) -> bool {
        match self {
            CrateTypes::Executable | CrateTypes::ProcMacro | CrateTypes::Sdylib => false,
            CrateTypes::Lib(tys) => tys.iter().all(|ty| match ty {
                LibCrateType::Rlib | LibCrateType::StaticLib => true,
                LibCrateType::Dylib | LibCrateType::Cdylib => false,
            }),
        }
    }

    pub fn sometimes_not_linked(&self) -> bool {
        match self {
            CrateTypes::Executable | CrateTypes::ProcMacro | CrateTypes::Sdylib => false,
    ...
    pub fn any_has_metadata(&self) -> bool {
        match self {
            CrateTypes::Executable | CrateTypes::Sdylib => false,
...
pub enum AttributeKind {
    ...
    CrateType(ThinVec<CliCrateType>),
