...
fn show_candidates(
    ...
    for path_strings in [&mut accessible_path_strings, &mut inaccessible_path_strings] {
        ...
        let core_path_strings =
            path_strings.extract_if(#[cfg(not(bootstrap))] (..), |p| p.0.starts_with("core::")).collect::<Vec<_>>();
        let std_path_strings =
            path_strings.extract_if(#[cfg(not(bootstrap))] (..), |p| p.0.starts_with("std::")).collect::<Vec<_>>();
        let foreign_crate_path_strings =
            path_strings.extract_if(#[cfg(not(bootstrap))] (..), |p| !p.0.starts_with("crate::")).collect::<Vec<_>>();
