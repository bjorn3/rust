...
fn get_backend_from_raw_matches(
    ...
    let host_sysroot =
        filesearch::materialize_sysroot(matches.opt_str("sysroot").map(PathBuf::from));
    let target = config::build_target_config(early_dcx, &target, &host_sysroot);

    get_codegen_backend(early_dcx, &host_sysroot, backend_name, &target)
}
